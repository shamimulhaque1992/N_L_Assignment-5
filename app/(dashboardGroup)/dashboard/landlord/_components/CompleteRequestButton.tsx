...
export default function CompleteRequestButton({
  ...
  const boundAction = (
    ...
    _formData: FormData,
  ) => updateRentalRequestStatus(prevState, rentalId, "COMPLETED");
        ...
        <DialogFooter>
          ...
          <form action={action}>
            <Button
              ...
              size="sm"
              variant={"outline"}
              className={`flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white hover:text-white`}
            >
              {pending ? (
              ...
              {pending ? "Processing…" : "Mark as completed"}
            </Button>
          </form>
        </DialogFooter>
      </DialogContent>
